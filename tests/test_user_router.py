...
class TestUserRouter(unittest.TestCase):
    ...
    def test_get_by_username(self, mock_get_by_username):
        ...
        response = self.client.get(f"/users/search/user1", headers=self.headers)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["username"], "user1")
        self.assertNotIn("password", response.json())
    ...
    def test_update_avatar(self, mock_set_avatar):
        mock_set_avatar.return_value = {"success": True, "message": "Avatar updated successfully"}
        # Send link as query parameter instead of JSON body
        response = self.client.put("/users/avatar/?link=avatar.jpg", headers=self.headers)
        self.assertEqual(response.status_code, 200)
